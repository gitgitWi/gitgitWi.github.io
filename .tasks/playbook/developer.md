# Developer Playbook (per-phase `pN-developer`)

> Model: `z-ai/glm-5.3-flash` (`cline -m z-ai/glm-5.3-flash`, act mode — NO `cline:` prefix, verified 2026-09-10).
> Implements the refined PLAN.md, opens the stacked PR.

## Startup brief (leader sends this)

```text
You are pN-developer for Phase N (<name>), branch feat/phase-N-<slug> (base: <parent>).
1. Read .tasks/phase-N-*/SPEC.md + refined PLAN.md + AGENTS.md + shared-code-style (7 rules).
2. Implement PLAN steps in order on your branch (create via `git checkout -b <branch> <parent>`).
